...
import { Upload, FileText, Link, Copy, Globe, Youtube } from 'lucide-react';
...
const AddSourcesDialog = ({
  ...
  const handleWebsiteUrlSubmit = async (url: string) => {
    ...
    try {
      // Create single website source record first to get the ID
      ...
      // Reuse the existing webhook with a single URL
      const { error } = await supabase.functions.invoke('process-additional-sources', {
        body: {
      ...
      toast({
        title: 'Success',
        description: 'Website added and sent for processing'
      });
    } catch (error) {
      console.error('Error adding website source:', error);
      toast({
        title: 'Error',
        description: 'Failed to add website source',
        variant: 'destructive'
      });
    } finally {
      setIsLocallyProcessing(false);
  ...
  const handleYouTubeUrlSubmit = async (url: string) => {
    ...
    try {
      // Create YouTube source record first to get the ID
      const createdSource = await addSourceAsync({
        notebookId,
        title: `YouTube: ${url}`,
        type: 'youtube',
      ...
      // Invoke processing webhook for YouTube
      const { error } = await supabase.functions.invoke('process-additional-sources', {
        body: {
          type: 'youtube',
          notebookId,
      ...
      toast({
        title: 'Success',
        description: 'YouTube video added and sent for processing'
      });
    } catch (error) {
      console.error('Error adding YouTube source:', error);
      toast({
        title: 'Error',
        description: 'Failed to add YouTube source',
        variant: 'destructive'
      });
    } finally {
      setIsLocallyProcessing(false);
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="pb-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <DialogTitle className="text-xl font-medium">Sail</DialogTitle>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-600 text-sm mb-1">Sources let Sail base its responses on the information that matters most to you.</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <span className="font-medium">Link - Single Website</span>
                <span className="text-sm text-gray-500">Add one URL</span>
              </Button>
              ...
              >
                ...
                <span className="text-sm text-gray-500">Add a video URL</span>
              </Button>
            </div>
          </div>
        </DialogContent>
